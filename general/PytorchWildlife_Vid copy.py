...
def classification_callback(frame: np.ndarray, results_det = None) -> np.ndarray:
    ...
    spp_dets = []
    for xyxy, tracker_id, normalized_coords in zip(
        results_det["detections"].xyxy, 
        results_det["detections"].tracker_id, 
        results_det['normalized_coords']):

        ## Save out cropped image as temp file
        cropped_image = sv.crop_image(image=frame, xyxy=xyxy)
        image = Image.fromarray(cropped_image)
        temp_dir = os.path.join(tempfile.gettempdir(), "SINClassifier")
        os.makedirs(temp_dir, exist_ok=True)
        img_path = os.path.join(temp_dir, results_det['img_id'] + ".png")
        image.save(img_path)

        spp_results = classification_model.single_image_classification(
            file_path = img_path, 
            det_results = {'img_id': img_path, 'normalized_coords': [normalized_coords]}, 
        )[0]

        spp_det = sv.Detections(
            xyxy = np.array([xyxy]),
            confidence = np.array([spp_results["confidence"]]), 
            class_id = np.array([spp_results["prediction"]]),
...
def vis_video(results, video_class, model, source_video_file, source_video_dir, target_dir, codec):
    ...
    full_output_dir = os.path.join(target_dir, relative_dir, str(video_class))
