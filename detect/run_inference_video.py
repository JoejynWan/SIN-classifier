...
def runtime_txt(
    ...
    lines = [
        ...
        'Runtime for manual identification vs MegaDetector detections comparison = {}'.format(
...
def get_arg_parser():
    ...
    parser.add_argument('--input_dir', type=str, 
                        ...
                        help = 'Path to folder containing the video(s) to be '
                                'processed.'
    )
    parser.add_argument('--recursive', type=bool, 
                        default = config.RECURSIVE, 
                        help = 'recurse into [input_dir]; only meaningful if a '
                                'folder is specified as input'
    )
    ...
    parser.add_argument('--delete_output_frames', type=bool,
                        ...
                        help = 'enable/disable temporary file deletion '
                                '(default True)'
    )
    ...
    parser.add_argument('--n_cores', type=int,
                        ...
                        help = 'number of cores to use for detection and '
                                'cropping (CPU only)'
    )
    ...
    parser.add_argument('--debug_max_frames', type=int,
                        ...
                        help = 'trim to N frames for debugging (impacts model '
                                'execution, not frame rendering)'
    )
    parser.add_argument('--check_accuracy', type=bool,
                        default = config.CHECK_ACCURACY, 
                        help = 'Whether accuracy of MegaDetector should be '
                                'checked with manual ID. Folder names must '
                                'contain species and quantity.'
    )
    parser.add_argument('--species_database_file', type=str,
                        default = config.SPECIES_DATABASE_FILE, 
                        help = 'Path to the species_database.csv which '
                                'describes details of species.'
    )
    parser.add_argument('--manual_id_csv', type=str,
                        default = config.MANUAL_ID_CSV, 
                        help = 'Path to csv file containing the results of '
                                'manual identification detections.'
    )
    ...
    parser.add_argument('--rolling_avg_size', type=float,
                        ...
                        help = 'Number of frames in which rolling prediction '
                                'averaging will be calculated from. A larger '
                                'number will remove more inconsistencies, but '
                                'will cause delays in detecting an object.'
    )
    parser.add_argument('--iou_threshold', type=float,
                        default = config.IOU_THRESHOLD, 
                        help = 'Threshold for Intersection over Union (IoU) to '
                                'determine if bounding boxes are detecting the '
                                'same object.'
    )
    parser.add_argument('--conf_threshold_buf', type=float,
                        default = config.CONF_THRESHOLD_BUF, 
                        help = 'Buffer for the rendering confidence threshold '
                                'to allow for "poorer" detections to be '
                                'included in rolling prediction averaging.'
    )
    parser.add_argument('--nth_highest_confidence', type=float,
                        default = config.NTH_HIGHEST_CONFIDENCE, 
                        help = 'nth-highest-confidence frame to choose a '
                                'confidence value for each video'
    )
    parser.add_argument('--resume_from_checkpoint', type=str,
                        default = config.RESUME_FROM_CHECKPOINT, 
                        help = 'path to JSON checkpoint file for which MD '
                                'detection will be resumed from.'
    )
    parser.add_argument('--checkpoint_path', type=str,
                        default = config.CHECKPOINT_PATH, 
                        help = 'path to JSON checkpoint file for which '
                                'checkpoints will be written to.'
    )
    parser.add_argument('--checkpoint_frequency', type=int,
                        default = config.CHECKPOINT_FREQUENCY, 
                        help = 'write results to JSON checkpoint file every '
                                'N images.'
    )
    return parser
